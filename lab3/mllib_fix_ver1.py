...
#temperature_file = temperature_file.sample(withReplacement=False, fraction=0.5)
...
for hour in hour_list:
    ...
    #calculate the threshold to filter out the data, since the train_data is already normalized, here the threshold is also normalized
    ...
    lr_model = LinearRegressionWithSGD.train(current_train_data, regType='l2', step=0.25, iterations = 13) 
    #Here, maxDepth is set to increase the split of model
    dt_model = DecisionTree.trainRegressor(current_train_data, categoricalFeaturesInfo={}, maxDepth= 10)
    
    lr_predictions = lr_model.predict(standardized_target_features).collect()[0]

    dt_predictions = dt_model.predict(standardized_target_features).collect()[0]
 
    prediction[hour]=(lr_predictions,dt_predictions)
