...
for hour in hour_list:
    ...
    #filter; here, it will keep data from previous date, and current date before current hour.
    temp_filtered = temperature_data_datekernel.filter(lambda x: datetime.strptime(x[1], '%H:%M:%S') <= target_hour_strip or datetime.strptime(x[4], '%Y-%m-%d') < target_date_strip)
